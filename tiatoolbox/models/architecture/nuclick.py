...
class NuClick(ModelABC):
    ...
    def __init__(self, num_input_channels: int = 5, num_output_channels: int = 1):
        super().__init__()
        self.net_name = "NuClick"
        if num_input_channels < 1:
            raise ValueError(f"input channels number error `{num_input_channels}`")
        self.n_channels = num_input_channels

        if num_output_channels < 1:
            raise ValueError(f"input channels number error `{num_output_channels}`")
